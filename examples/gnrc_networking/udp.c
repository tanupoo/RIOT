                 ...
                 unsigned int delay)
{
    ...
    for (unsigned int i = 0; i < num; i++) {
        ...
        if (udp == NULL) {
            ...
            printf("%s,%u release size: %u\n", RIOT_FILE_RELATIVE, __LINE__, payload->size); gnrc_pktbuf_release(payload);
            return;
        }
        ...
        if (ip == NULL) {
            ...
            printf("%s,%u release size: %u\n", RIOT_FILE_RELATIVE, __LINE__, udp->size); gnrc_pktbuf_release(udp);
            return;
        }
        /* send packet */
        if (!gnrc_netapi_dispatch_send(GNRC_NETTYPE_UDP, GNRC_NETREG_DEMUX_CTX_ALL, ip)) {
            puts("Error: unable to locate UDP thread");
            printf("%s,%u release size: %u\n", RIOT_FILE_RELATIVE, __LINE__, ip->size); gnrc_pktbuf_release(ip);
            return;
        }
