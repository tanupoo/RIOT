...
/* transceiver stack */
long long transceiver_stack[TRANSCEIVER_STACK_SIZE];
...
{
    transceiver_pid = thread_create(transceiver_stack, sizeof(transceiver_stack), PRIORITY_MAIN - 3, CREATE_STACKTEST, run, NULL, "Transceiver");
    ...
    if (transceiver_pid < 0) {
        ...
        DEBUGF("transceiver: Transceiver started for CC1100\n");
        ...
        DEBUGF("transceiver: Transceiver started for CC2420\n");
        ...
        DEBUGF("transceiver: Transceiver started for AT86RF231\n");
...
uint8_t transceiver_register(transceiver_type_t t, int pid)
{
    ...
    for (size_t i = 0; i < TRANSCEIVER_MAX_REGISTERED; i++) {
        if ((reg[i].pid == pid) || (reg[i].transceivers == TRANSCEIVER_NONE)) {
            ...
            reg[i].pid = pid;
            DEBUGF("transceiver: Thread %i registered for %i\n", reg[i].pid, reg[i].transceivers);
            restoreIRQ(state);
            result = 1;
            break;
        }
...
static void *run(void *arg)
{
    ...
    while (1) {
        DEBUGF("transceiver: Waiting for next message\n");
        msg_receive(&m);
        /* only makes sense for messages for upper layers */
        cmd = (transceiver_command_t *) m.content.ptr;
        DEBUGF("transceiver: Transceiver: Message received, type: %02X\n", m.type);
        ...
        switch (m.type) {
            ...
            default:
                DEBUGF("transceiver: Unknown message received\n");
                break;
        }
    }
...
static void receive_packet(uint16_t type, uint8_t pos)
{
    ...
    while (reg[i].transceivers != TRANSCEIVER_NONE) {
        if (reg[i].transceivers & t) {
            ...
            DEBUGF("transceiver: Notify thread %i\n", reg[i].pid);
...
static void receive_cc110x_packet(radio_packet_t *trans_p)
{
    DEBUGF("transceiver: Handling CC1100 packet\n");
...
{
    DEBUGF("transceiver: Handling CC2420 packet\n");
    dINT();
    ...
    if (trans_p->frame.fcf.dest_addr_m == IEEE_802154_SHORT_ADDR_M) {
        ...
        DEBUGF("Illegal destination address mode: %d\n", trans_p->frame.fcf.src_addr_m);
        return;
    }
#endif
    trans_p->frame.payload = (uint8_t *) &(data_buffer[transceiver_buffer_pos * CC2420_MAX_DATA_LENGTH]);
    trans_p->frame.payload_len = p->frame.payload_len;
    DEBUGF("transceiver: Content: %s\n", trans_p->data);
}
#endif
...
void receive_nativenet_packet(radio_packet_t *trans_p)
{
    ...
    DEBUGF("Handling nativenet packet\n");
    ...
    DEBUGF("Packet %p was from %" PRIu16 " to %" PRIu16 ", size: %" PRIu8 "\n", trans_p, trans_p->src, trans_p->dst, trans_p->length);
...
void receive_at86rf231_packet(ieee802154_packet_t *trans_p)
{
    DEBUGF("Handling AT86RF231 packet\n");
    dINT();
...
static int8_t send_packet(transceiver_type_t t, void *pkt)
{
    ...
    DEBUGF("transceiver: Send packet to ");
#ifdef DEBUG_ENABLED
    ...
    DEBUGF("transceiver: Send packet to %" PRIu16 "\n", p->dst);
    ...
    switch (t) {
        case TRANSCEIVER_CC1100:
            ...
            DEBUGF("transceiver: snd_ret (%u) = %i\n", p->length, res);
#else
...
static int16_t ignore_add(transceiver_type_t transceiver, void *address)
{
    ...
    for (size_t i = 0; i < TRANSCEIVER_MAX_IGNORED_ADDR; i++) {
        if (transceiver_ignored_addr[i] == 0) {
            ...
            DEBUGF("addr %u will be ignored (%u)\n", addr, i);
            return i;
        }
    }
