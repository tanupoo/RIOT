                             ...
                             uint8_t *data, uint16_t data_len)
{
    ...
    uint16_t my_packet_length = data_len;
    ...
    DEBUG("sixlowpan_lowpan_sendto(%s, data, %"PRIu16"): my_packet_length: %"PRIu16", header_size: %"PRIu16"\n",
            sixlowpan_mac_802154_long_addr_to_str(addr_str, dest), data_len, my_packet_length, header_size);
    if (my_packet_length + header_size > PAYLOAD_SIZE - IEEE_802154_MAX_HDR_LEN) {
        uint8_t fragbuf[my_packet_length + header_size];
        uint8_t remaining;
        uint8_t i = 2;
        /* first fragment */
        ...
        fragbuf[0] = ((SIXLOWPAN_FRAG1_DISPATCH << 8) | my_packet_length) >> 8;
        fragbuf[1] = (SIXLOWPAN_FRAG1_DISPATCH << 8) | my_packet_length;
        fragbuf[2] = tag >> 8;
        fragbuf[3] = tag;
        ...
        while (my_packet_length - position > max_frame - 5) {
            memset(&fragbuf, 0, my_packet_length + header_size);
            memcpy(fragbuf + 5, data, max_frag);

            fragbuf[0] = ((SIXLOWPAN_FRAGN_DISPATCH << 8) | my_packet_length) >> 8;
            fragbuf[1] = (SIXLOWPAN_FRAGN_DISPATCH << 8) | my_packet_length;
            fragbuf[2] = tag >> 8;
            fragbuf[3] = tag;
        ...
        remaining = my_packet_length - position;
