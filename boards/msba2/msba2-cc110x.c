...
void cc110x_enable_interrupt(uint8_t pin, uint8_t flags) 
{
   switch (pin) {
        case GDO0:
            switch (flags) {
                case HIGH:
                    gpioint_set(0, BIT27, GPIOINT_RISING_EDGE, 
                                       &cc110x_gdo0_interrupt);
                    break;
                case LOW:
                    gpioint_set(0, BIT27, GPIOINT_FALLING_EDGE, 
                                       &cc110x_gdo0_interrupt);
                    break;
                default:
                    break;
        ...
        case GDO2:
             switch (flags) {
                case HIGH:
                    gpioint_set(0, BIT28, GPIOINT_RISING_EDGE, 
                                      &cc110x_gdo2_interrupt);
                    break;
                case LOW:
                    FIO0DIR &= ~BIT28;
                    gpioint_set(0, BIT28, GPIOINT_FALLING_EDGE, 
                                        &cc110x_gdo2_interrupt);
                    break;
                default:
                    break;
...
void cc110x_clear_interrupt(uint8_t pin) {
    switch (pin) {
        case GDO0:
            gpioint_clear_interrupt(0, BIT27);
            break;
        case GDO2:
            gpioint_clear_interrupt(0, BIT28);
            break;
        default:
            break;
    }
