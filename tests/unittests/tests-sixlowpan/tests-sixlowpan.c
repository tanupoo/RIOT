...
static void test_sixlowpan_init_success(void)
{
    ...
    TEST_ASSERT((sl_pid != EOVERFLOW) && (sl_pid != KERNEL_PID_UNDEF));
}

/* XXX: API documentation is not documenting this case */
static void test_sixlowpan_init_twice(void)
{
    ...
    TEST_ASSERT(sl_pid1 != sl_pid2);
    TEST_ASSERT(sl_pid2 != EOVERFLOW);
    TEST_ASSERT(sl_pid2 != KERNEL_PID_UNDEF);
}

/* TODO: if multiple initialization is possible -> initialize until out of memory */
...
Test *test_sixlowpan_tests(void)
{
    ...
    EMB_UNIT_TESTCALLER(test_sixlowpan_tests_caller, set_up, tear_down, fixtures);
